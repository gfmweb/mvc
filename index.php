...
// Автозагрузчик классов
    spl_autoload_register(function($className) {
        ...
        if(file_exists($pieces[0]."/".$pieces[1] .'.php')){ // проверка существования контроллера
                ...
                Header('Location: /404.php?er_title='.urlencode("УПС! Что-то пошло совсем не так").'&description='.urlencode("Нет такого раздела на этом сайте ".$pieces[0]."/".$pieces[1]).'&action='.urlencode("<a href=\"http://".$_SERVER['SERVER_NAME']."\">Вернуться на сайт</a>"));
        }
    });
    ...
    if($check_magic==="core\Magic"){ // Если контроллер является наследником Магического класса
        ...
        if(method_exists($implement_controller,$method)){ // Проверка существования метода этого контроллера
            ...
            Header('Location: /404.php?er_title='.urlencode("УПС! Что-то пошло немного не так").'&description='.urlencode("Нет такой странички на этом сайте").'&action='.urlencode("<a href=\"http://".$_SERVER['SERVER_NAME']."\">Вернуться на главную</a><hr/><a href=\"http://".$_SERVER['SERVER_NAME']."\" class='mt-3'>Вернуться в раздел</a>"));
        }
    }
